...
namespace GameService.Client.Sdk.Repositories.Services.Device
{
    ...
    {
        public async Task<RowListResponse<Sdk.Services.Services.User.Device>> FindMyDevices<T>(T input) where T : FindMyDeviceParams
        {
            ...
            {
                return JsonConvert.DeserializeObject<RowListResponse<Sdk.Services.Services.User.Device>>(body);
            }
            else
            {
