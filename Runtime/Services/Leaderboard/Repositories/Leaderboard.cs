...
namespace DynamicPixels.GameService.Services.Leaderboard.Repositories
{
    public class LeaderboardRepository : ILeaderboardRepository
    {
        ...
        public Task<RowListResponse<Models.Leaderboard>> GetLeaderBoards<T>(T input) where T : GetLeaderboardsParams
        {
        ...
        public Task<RowListResponse<TOutput>> GetPartiesScores<TInput, TOutput>(TInput input)
        ...
        public Task<RowListResponse<TOutput>> GetUsersScores<TInput, TOutput>(TInput input)
            where TInput : GetScoresParams
        ...
        public Task<RowResponse<TOutput>> GetCurrentUserScore<TInput, TOutput>(TInput input)
       ...
       where TOutput : UserScore
        ...
        public Task<RowListResponse<UserScore>> GetFriendsScores<T>(T input) where T : GetFriendsScoresParams
        {
            return WebRequest.Get<RowListResponse<UserScore>>(UrlMap.GetMyFriendsScoreUrl(input.LeaderboardId));
        }

        public Task<RowResponse<TOutput>> SubmitScore<TInput, TOutput>(TInput input)
