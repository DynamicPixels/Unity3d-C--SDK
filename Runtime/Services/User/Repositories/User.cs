...
namespace DynamicPixels.GameService.Services.User.Repositories
{
    ...
    {
        public Task<RowListResponse<Models.User>> Find<T>(T input) where T : FindUserParams
        {
            return WebRequest.Post<RowListResponse<Models.User>>(UrlMap.FindUsersUrl, input.ToString());
        }

        public Task<RowResponse<Models.User>> FindUserById<T>(T input) where T : FindUserByIdParams
        {
            return WebRequest.Get<RowResponse<Models.User>>(UrlMap.FindUserByIdUrl(input.UserId));
        }

        public Task<RowResponse<Models.User>> EditCurrentUser<T>(T input) where T : EditCurrentUserParams
        {
